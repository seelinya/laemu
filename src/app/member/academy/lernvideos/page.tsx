      ...
      >
        ...
        <div className="flex-1 p-4 flex flex-col sm:flex-row gap-3 sm:gap-4 min-w-0">
          ...
          <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between gap-2 flex-shrink-0 border-t sm:border-t-0 border-border pt-3 sm:pt-0 mt-1 sm:mt-0">
            ...
            {unlocked ? (
              <Link href={`/member/academy/lernvideos/${v.id}`} className="block font-sans text-xs px-3 py-1.5 transition-colors whitespace-nowrap text-center bg-dark text-white hover:bg-accent-gold hover:text-white">
                Öffnen →
              </Link>
            ) : (
        ...
        right={
          userAbo.plan === 'none' ? (
            <Link
              ...
              className="flex items-center gap-2 bg-accent-gold text-white border border-accent-gold px-3 sm:px-4 py-1.5 font-sans font-semibold text-sm hover:bg-accent-gold-hover transition-colors"
            >
        ...
        {tab === 'wuensche' && (
          <div className="max-w-3xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowWishForm(!showWishForm)} className="bg-accent-gold text-white font-sans text-sm px-4 py-2 hover:bg-accent-gold-hover transition-colors flex-shrink-0">
                + Neuer Wunsch
              </button>
            </div>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mb-6">
                  <div className="bg-surface border border-accent-gold/30 p-6 space-y-4">
                    ...
                    <div className="flex gap-3">
                      <button onClick={submitWish} disabled={!wishTitle.trim() || wishVoteSel.length === 0} className="bg-accent-gold text-white font-sans text-sm px-5 py-2 hover:bg-accent-gold-hover transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Wunsch einreichen</button>
