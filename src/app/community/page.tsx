          ...
          <Section className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <motion.h2 variants={fadeUp} className="heading-lg text-white mt-3 mb-5">
                Instrumente, Schulen & Fachgeschäfte.
              </motion.h2>
              <motion.p variants={fadeUp} className="body-lg text-white/60 mb-8">
                Von Instrumentenbauern über Ländlerlokale bis zu Vereinen und Stiftungen — das LAEMU-Partnernetzwerk verbindet die ganze Szene.
              </motion.p>
              <motion.div variants={fadeUp}>
                <Button href="/partner" variant="primary" size="md">Partner entdecken →</Button>
              </motion.div>
            </div>
            <motion.div variants={fadeUp} className="grid grid-cols-2 gap-3">
              {[
                { label: 'Instrumentenbauer', count: '12' },
                { label: 'Fachgeschäfte', count: '8' },
                { label: 'Musikschulen', count: '15' },
                { label: 'Ländlerlokale', count: '24' },
              ].map(item => (
                <div key={item.label} className="border border-white/10 p-5 text-center hover:border-accent-gold transition-colors">
                  <p className="font-heading font-bold text-3xl text-accent-gold">{item.count}</p>
                  <p className="font-sans text-xs text-white/50 mt-1">{item.label}</p>
                </div>
              ))}
            </motion.div>
          </Section>
